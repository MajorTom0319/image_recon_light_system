...
def optimize_envmap_ARMN(scene,cam_cfg,mat,save_folder,use_mesh_normal,
    ...
    if final_spp > spp:
        ...
        if use_mesh_normal:
            final_img = render_w_brdf(scene, mat['albedo'], mat['roughness'], mat['metallic'], None, final_spp)
        else:
            final_img = render_w_brdf(scene, mat['albedo'], mat['roughness'], mat['metallic'], mat['normal'], final_spp)
        hq_path = os.path.join(output_dir, 'best_results', 'rendered_img_hq.exr')
        mi.util.write_bitmap(hq_path, final_img.detach().cpu().numpy())
