...
python optimal_lights/optimize_hemisphere_uniform_lights_qoder_modifily.py \
    --target_path /home/majortom/project/Materialist/output_imgs/interiorverse_testinfer_moge2/gt_image.png \
    --mesh_path /home/majortom/project/Materialist/output_imgs/interiorverse_testinfer_moge2/mesh_moge2.ply \
    --mat_dir /home/majortom/project/Materialist/output_imgs/interiorverse_testinfer_moge2 \
    --camera_meta /home/majortom/project/Materialist/output_imgs/interiorverse_testinfer_moge2/camera_meta.json \
    --output_dir /home/majortom/project/Materialist/output_imgs/interiorverse_testinfer_moge2/light_optimization_uniform_light \
    --num_lights 32 \
    --spot_count 8 \
    --iterations 800 \
    --spp 32 \
...
def build_scene(
    ...
    """
    Build Mitsuba scene with point lights + optional additional spotlights.
...
def optimize_lights(args: argparse.Namespace) -> None:
    ...
    # =========================================================================
    # JOINT MODE: Single-phase optimization with mixed point+spot lights
    # =========================================================================
    if args.joint and args.spot_count > 0:
        print("\n" + "=" * 70)
        print("JOINT MODE: Mixed point + spot lights from the start")
        print("=" * 70)

        # Generate spotlight positions and directions
        # Place spotlights at high-elevation positions, aimed at scene center
        spot_positions_joint: list[np.ndarray] = []
        spot_directions_joint: list[np.ndarray] = []

        # Use Fibonacci spiral for spotlight positions (different from point lights)
        spot_offset = args.num_lights  # offset to get different positions
        for i in range(args.spot_count):
            # High elevation positions for spotlights
            idx = spot_offset + i
            y = 0.7 + 0.25 * (i + 0.5) / args.spot_count  # elevation 0.7-0.95
            ...
            # Aim at scene center with some angular diversity
            aim_offset = np.array([
                math.sin(azimuth * 2) * 0.3,
                -0.2,
                math.cos(azimuth * 3) * 0.3,
            ], dtype=np.float32)
        ...
        # Build scene with both point lights and spotlights
        scene = build_scene(
        ...
        for iteration in range(args.iterations):
            ...
            if iteration % args.log_interval == 0 or iteration == args.iterations - 1:
                ...
                print(
                    ...
                    f"spot=[{spot_sum[0]:.1f},{spot_sum[1]:.1f},{spot_sum[2]:.1f}]"
                )
